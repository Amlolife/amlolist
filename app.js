document.addEventListener('DOMContentLoaded', () => {
    ...
    const render = {
        ...
        projects: () => {
            ...
            container.innerHTML = `<div class="p-6"><h1 class="text-3xl font-bold">Projects</h1><p class="text-gray-400">Select a project to view its schedule</p></div><div class="px-6 grid grid-cols-2 gap-4">${listHtml}</div><div class="p-6"><button data-target="page-add-project" class="w-full flex items-center justify-center rounded-lg h-12 px-5 bg-purple-600 hover:bg-purple-500 text-white text-base font-bold">Create New Project</button></div>`;
            animateList(container.querySelector('.grid'));
        },
        addProject: () => {
            const container = pageContainers.addProject;
            if (!container) return;
        ...
        shotList: () => {
            ...
            container.innerHTML = `<div id="shot-list-header">${headerHtml}</div><div id="shot-list-container" class="px-6 flex flex-col gap-4">${listHtml}</div>`;
            animateList(container.querySelector('#shot-list-container'));
        },
        addShot: () => {
        ...
        settings: () => {
            ...
            if(!container) return;
            container.querySelector('#dark-mode-toggle').checked = appState.settings.isDarkMode;
        },
    };
    ...
    document.getElementById('add-project-form')?.addEventListener('submit', (e) => {
        e.preventDefault();
        const title = e.target.querySelector('#project-title').value;
        const category = e.target.querySelector('#project-category').value;
        const date = e.target.querySelector('#project-date').value;
    ...
    document.getElementById('add-shot-form')?.addEventListener('submit', e => {
        ...
        const title = e.target.querySelector('#task-title').value;
        const time = e.target.querySelector('#task-time').value;
        const category = e.target.querySelector('.category-pill.active')?.dataset.category || 'Personal';
